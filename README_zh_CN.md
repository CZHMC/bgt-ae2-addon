...
一个为 Building Gadgets 2 接入 Applied Energistics 2 自动合成流程的附属模组。当建筑队列缺少材料时，本模组会把材料列表交给 AE2 原生的数量窗口和合成计划窗口；网络中有样板的材料会进入合成计划，没有样板的材料会直接显示为缺少数量。
...
- 不要求每种材料都有合成样板；没有样板的材料也会进入同一批请求，并在 AE2 计划中显示缺少数量。
...
2. 将需要自动合成的材料对应样板放入 AE2 网络；没有样板的材料也可以继续请求，计划中会列出缺少数量。
...
AE2 的单个合成计划一次只能请求一种物品。缺少多种不同材料时，原生数量窗口会按顺序展示这些材料；有样板的材料会打开正常合成计划，没有样板的材料会生成列出缺少数量的部分计划。在整个顺序完成前，本模组会保持 BGT 整批队列暂停。
